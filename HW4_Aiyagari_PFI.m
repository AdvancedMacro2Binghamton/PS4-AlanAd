...
while abs(K_dist) >= .01
   ...
   % Get Q-Matrix for iteration step
   Q= makeQmatrix(pol_indx,PI);
   
   p_fn = a(pol_indx);
   
   u_mat= bsxfun(@minus, r*a, p_fn);
...
figure(2)
 ...
 r_cm= 1/beta % Competitve Market Rate
 r  % Model Rate
 
 
% Wealth Distribution and GINI

population= reshape(Mu',[num_z*num_a,1]);
